...
class WynnBot extends CI_Controller {
	...
	public function index1()
	{
        ...
        $this->bot = new LINEBot($httpClient, array('channelSecret' => $this->config->item('LINE_CHANEL')));
           ...
           $this->log(
			...
			if($inputType == 'message'){
                ...
                if($messageType == 'text'){
                    ...
                    switch($param_arg[0]):
                        case 'member':
                            if(isset($param_arg[1])){
                                $this->member($param_arg[1]);
                            }
                        break;                        
                        case 'program':
                            if(isset($param_arg[1])){
                                $this->program($param_arg[1]);
                            }
                        break;
                        case 'ดูหนังออนไลน์':
                            $this->movie();
    ...
    private function setting_service(){
        ...
        $imageMapUrl = "https://ktdev.site/LINE_BOT_CI/assets/images/setting.jpg?_ignored=";
    ...
    private function promotion(){
        ...
        $imageMapUrl = 'https://ktdev.site/LINE_BOT_CI/assets/images/promotion-new1.jpg?_ignored=';
        $textReplyMessage = new ImagemapMessageBuilder(
            $imageMapUrl,
            'โปรโมชั่นสินค้า',
    ...
    private function movie(){
        ...
        $movieResult = $this->BOT_model->getURLList('MOVIE');
    ...
    private function shopping(){
        ...
        $shopResult = $this->BOT_model->getURLList('SHOP');
    ...
    private function get_member(){
        ...
        if($this->member){
            ...
            $this->member = $this->BOT_model->get_member($this->UID);
            return $this->member;
        }
    }

    private function log($data){
        $this->BOT_model->insert_log($data);
    }
